...
export function registerApplication(appName, applicationOrLoadFunction, activityWhen, customProps = {}) {
    ...
    // promise 化，统一应用形式
    let applicationOrLoadFunctionCb = applicationOrLoadFunction
    if (typeof applicationOrLoadFunction !== 'function') {
        applicationOrLoadFunctionCb = () => Promise.resolve(applicationOrLoadFunction);
    }
    APPS.push({
        name: appName,
        loadApp: applicationOrLoadFunctionCb,
        activityWhen,
        customProps,
        status: NOT_LOADED,
        services: {}
    });
    // 调用 loadApps 将对应的已经可以启动的 app 进行扁平化
    return invoke();
}
